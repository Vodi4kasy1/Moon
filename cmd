...
do
    function Utility:Log(Type, Message)
        Type = Type:lower()
        if Type == 'error' then
            error('[ MoonLight ] Error: ' .. Message)
        elseif Type == 'log' then
            print('[ MoonLight ] ' .. Message)
        elseif Type == 'warn' then
            warn('[ MoonLight ] Warning: ' .. Message)
        end
    end
...
function Library:CreateWindow(Properties)
    local Name = Utility:GetProperty('Window', 'Name', Properties) or 'MoonLight UI'
    local IntroText = Utility:GetProperty('Window', 'IntroText', Properties) or 'MoonLight UI'
    ...
    if IntroBlur then
        Utility:Create('BlurEffect', {
            Name = 'MoonLightIntroBlur',
            Parent = Lighting,
            Size = 0
        })
        local Blur = Lighting.MoonLightIntroBlur
    ...
    task.wait(0.5)
    if IntroBlur then
        Utility:Tween(Lighting.MoonLightIntroBlur, {Size = 0}, 1)
        task.wait(1)
    end
    Main['IntroText']:Destroy()
    Main['IntroImage']:Destroy()
    if IntroBlur then
        Lighting.MoonLightIntroBlur:Destroy()
    end
    ...
    local function UpdateFrameSizes()
        ...
        elseif Count < 9 then
            ...
            Utility:Tween(CommandsHolderScrolling, {CanvasSize = UDim2.new(0, 500, 0, Size)}, 0.25)
            if string.find(Position, 'bottom') then
                Utility:Tween(CommandsHolder, {Position = UDim2.new(0, 0, 0, -Size - 13)}, 0.25)
            else
                Utility:Tween(CommandsHolder, {Position = UDim2.new(0, 0, 0, 70)}, 0.25)
            end
        else
            ...
            Utility:Tween(CommandsHolderScrolling, {CanvasSize = UDim2.new(0, 500, 0, Size)}, 0.25)
            if string.find(Position, 'bottom') then
                Utility:Tween(CommandsHolder, {Position = UDim2.new(0, 0, 0, -273)}, 0.25)
            else
                Utility:Tween(CommandsHolder, {Position = UDim2.new(0, 0, 0, 70)}, 0.25)
            end
        end
    ...
    UserInputService.InputBegan:Connect(function(Input, GameProcessedEvent)
        if not GameProcessedEvent and Input.KeyCode == Library.Prefix then
            UpdateFrameSizes()
    ...
    local function Execute(String)
        ...
        if not Command then
            Utility:Log('Error', 'Command Not Found: ' .. First)
            return
        end
    ...
    CommandInput:GetPropertyChangedSignal('Text'):Connect(function()
        if CommandInput.Text == '' then
            ...
            for _, Instance in next, CommandsHolderScrolling:GetChildren() do
                if not Instance:IsA('UIListLayout') then
                    local cmdName = Instance.Name:gsub('CommandHolder', ''):lower()
                    local aliases = Commands[cmdName].Aliases or {}
                    local matches = string.find(cmdName, CommandInput.Text:lower())
                    for _, alias in ipairs(aliases) do
                        if string.find(alias:lower(), CommandInput.Text:lower()) then
                            matches = true
                            break
                        end
                    end
    ...
    function CreateToolTip(Text, Clone, Enabled)
        ...
        if Enabled then
            ...
            RunService:BindToRenderStep('ToolTip', 1, function()
                ...
                local Offset = UDim2.new(0, 10, 0, 10)
    ...
    function WindowFunctions:AddCommand(Name, Aliases, Arguments, Description, Callback)
        local commandData = {
            Name = Name:lower(),
            Aliases = Aliases or {},
            Arguments = Arguments,
